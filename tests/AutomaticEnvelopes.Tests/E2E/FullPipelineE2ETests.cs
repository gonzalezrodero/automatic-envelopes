...
public class FullPipelineE2ETests(IntegrationAppFixture fixture)
{
    ...
    public async Task FullRAGJourney_FromIngestionToAIResponse()
    {
        ...
        // --- 4. Assert ---
        // FIX: We must wait for the 'ReplyGenerated' specifically, because the Saga takes 10 seconds
        await WaitForConditionAsync(async () =>
        {
            using var session = fixture.Host.Services.GetRequiredService<IDocumentStore>().LightweightSession(tenant);
            var evts = await session.Events.FetchStreamAsync(testPhoneNumber);
            return evts.Any(e => e.Data is ReplyGenerated);
        }, "The bot reply was never generated by the Saga.", timeoutSeconds: 25);
    ...
    public async Task Webhook_Idempotency_DuplicateMessages_AreIgnored()
    {
        ...
        }, "First message was never processed by the background worker.", timeoutSeconds: 25);
    ...
    public async Task GdprDeletion_WhenUserRequestsDeletion_StreamIsRemovedAndDataAnonymized()
    {
        ...
        // FIX: Wait for the FULL 10-second Saga to process the first message and generate a reply
        await WaitForConditionAsync(async () =>
        {
            using var session = fixture.Host.Services.GetRequiredService<IDocumentStore>().LightweightSession(tenant);
            var evts = await session.Events.FetchStreamAsync(testPhoneNumber);
        ...
        // FIX: The deletion command is inside the 2nd Saga, so we must wait up to 25 seconds for it to run
        await WaitForConditionAsync(async () =>
        {
            using var session = fixture.Host.Services.GetRequiredService<IDocumentStore>().LightweightSession(tenant);
            var evts = await session.Events.FetchStreamAsync(testPhoneNumber);
    ...
    public async Task RapidSequentialMessages_AreGroupedBySaga_AndGenerateASingleReply()
    {
        ...
        // Wait for all 3 messages to be successfully appended to the Marten stream
        await WaitForConditionAsync(async () =>
        {
            using var session = fixture.Host.Services.GetRequiredService<IDocumentStore>().LightweightSession(tenant);
            var evts = await session.Events.FetchStreamAsync(testPhoneNumber);
            return evts.Count(e => e.Data is MessageReceived) >= 3;
        }, "The 3 rapid messages were not saved to the database.", timeoutSeconds: 15);
